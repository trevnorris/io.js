...
static void SetupHooks(const FunctionCallbackInfo<Value>& args) {
  ...
  Local<Value> init_v =
      fn_obj->Get(FIXED_ONE_BYTE_STRING(env->isolate(), "init"));
  Local<Value> pre_v =
      fn_obj->Get(FIXED_ONE_BYTE_STRING(env->isolate(), "pre"));
  Local<Value> post_v =
      fn_obj->Get(FIXED_ONE_BYTE_STRING(env->isolate(), "post"));
  Local<Value> final_v =
  ...
  if (final_v->IsFunction())
    env->set_async_hooks_final_function(final_v.As<Function>());
